...
import { BsBattery, BsSpeedometer2 } from "react-icons/bs";
import {
  MdLocalGasStation,
  MdLock,
  MdNavigation,
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        "https://vechilebackend.onrender.com/api/vehicle"
      );
...
useEffect(() => {
  if (isMoving && routeData.length > 1) {
    const moveVehicle = async () => {
      try {
        const currentIndex = routeData.findIndex((point) => 
          Math.abs(point.latitude - currentPosition.coords[0]) < TOLERANCE &&
        ...
        const nextIndex = Math.min(currentIndex + steps, routeData.length - 1);
        ...
        if (!nextPosition || nextPosition.latitude === undefined || nextPosition.longitude === undefined) {
  ...
  const handleRouteChange = async (e, initialLoad = false) => {
    ...
    try {
      setLoading(true);
      await axios.post("https://vechilebackend.onrender.com/api/select-route", {
        routeName,
      });
      const response = await axios.get(
        "https://vechilebackend.onrender.com/api/vehicle"
